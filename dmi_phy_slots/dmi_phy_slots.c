...
static void dmi_decode(const struct dmi_header *h, struct dmi_physlot_bus_pair *item)
{
	...
	item->phy_slot = dmi_string(h, data[0x04]); 
	if (h->length < 0x11)
		return;
	// Step 2 - set bus addr
...
static void dmi_table_decode(u8 *buf, u32 len, u16 num,u32 flags, struct dmi_physlot_bus_pair **head)
{
	    ...
	    && data + 4 <= buf + len) /* 4 is the length of an SMBIOS structure header */
	{
		...
		if (h.type == 9){
			if(!*head){
				if(!(*head=curr=calloc(1, sizeof(struct dmi_physlot_bus_pair)))){
					perror("calloc");
					return;
				}
			}else{
				if(!(curr->next=calloc(1, sizeof(struct dmi_physlot_bus_pair)))){
					perror("calloc");
					return;
				}
				curr=curr->next;
...
static void dmi_table(off_t base, u32 len, u16 num, u32 ver, const char *devmem,
		      u32 flags, struct dmi_physlot_bus_pair **head)
{
	/* Dependencies */
...
static int smbios3_decode(u8 *buf, const char *devmem, u32 flags, struct dmi_physlot_bus_pair **head)
{
	/* Dependencies */
	u32 ver;
	u64 offset;
...
static int smbios_decode(u8 *buf, const char *devmem, u32 flags, struct dmi_physlot_bus_pair **head)
{
	u16 ver;
...
static int legacy_decode(u8 *buf, const char *devmem, u32 flags, struct dmi_physlot_bus_pair **head)
{
...
int dmi_fill_physlot_bus_pairs(struct dmi_physlot_bus_pair **table){
	...
	if((buf = read_file(0, &size, SYS_ENTRY_FILE)) != NULL)
	{
		...
		{
			if (smbios_decode(buf, SYS_TABLE_FILE, FLAG_NO_FILE_OFFSET, table))
				found++;
		}
		else if (size >= 15 && memcmp(buf, "_DMI_", 5) == 0)
		{
			if (legacy_decode(buf, SYS_TABLE_FILE, FLAG_NO_FILE_OFFSET, table))
				found++;
		}
...
/*
	...
	{
		if (smbios3_decode(buf, devmem, 0, table))
			found++;
	}
	else if (memcmp(buf, "_SM_", 4) == 0)
	{
		if (smbios_decode(buf, devmem, 0, table))
			found++;
	}
	goto done;
...
memory_scan:
	...
	for (fp = 0; fp <= 0xFFE0; fp += 16)
	{
		...
		{
			if (smbios3_decode(buf + fp, devmem, 0, table))
			{
				found++;
				goto done;
			}
	...
	for (fp = 0; fp <= 0xFFF0; fp += 16)
	{
		...
		{
			if (smbios_decode(buf + fp, devmem, 0, table))
			{
				found++;
				goto done;
			}
		...
		{
			if (legacy_decode(buf + fp, devmem, 0, table))
			{
				found++;
				goto done;
			}
